...
void i2c_init(i2c_master_dev_handle_t *dev_handle) {
    ...
    if (MPU6050_ADDR == read_buf) {
        printf("I2C Successfully connected to MPU6050 at address 0x%X\n", (unsigned int) read_buf);
    } else {
        printf("I2C failed to establish a connection, received address %X, expected %X\n", (unsigned int) read_buf, MPU6050_ADDR);
    }
}

void mpu6050_init_cfg(i2c_master_dev_handle_t *dev_handle) {
    i2c_write_reg(*dev_handle, SMPRT_DIV_REG, 0b0, 1); // smprt = gyro_rt / (1 + div)
}
